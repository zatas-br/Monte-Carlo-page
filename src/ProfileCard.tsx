...
const GOLD_INNER_GRADIENT = 'linear-gradient(135deg, #a48111 0%, #f9e278 25%, #d4af37 50%, #f9e278 75%, #a48111 100%)';
...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  // If attributes are present, we switch to FIFA mode
  const isFifaCard = !!attributes;
  const activeInnerGradient = innerGradient ?? (isFifaCard ? GOLD_INNER_GRADIENT : DEFAULT_INNER_GRADIENT);
  ...
  const cardStyle = useMemo(
    () => ({
      ...
      '--inner-gradient': activeInnerGradient,
    ...
    [iconUrl, grainUrl, activeInnerGradient, behindGlowColor, behindGlowSize, cardRadius]
  );
